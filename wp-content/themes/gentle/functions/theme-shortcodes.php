...
function mpc_recent_news($atts, $content = null) {
	extract(shortcode_atts(array(
    'number' => '4',
    'post' => ''  
    ), $atts));
...
add_shortcode('mpc_recent_news', 'mpc_recent_news');

/*-----------------------------------------------------------------------------------*/
/*	X. Recent Projects
/*-----------------------------------------------------------------------------------*/

function mpc_recent_projects($atts, $content = null) {
	extract(shortcode_atts(array(
    'number' => '4',
    'post' => ''  
    ), $atts));
...
// print_r('</pre>');
	 ...
	 <script type="text/javascript">
		...
		jQuery(document).ready(function() {
		    jQuery('#gentle_latest_projects_jcarousel').jcarousel({
		    	wrap: 'circular',
		    	scroll: <?php echo $scroll; ?>,
		    	easing: 'easeOutExpo',
		    	animation: 'slow',
